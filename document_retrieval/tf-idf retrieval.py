...
class TfidfDocRanker(object):
    ...
    def hash(self, word, hash_size):
        return murmurhash3_32(word, positive=True) % hash_size
    ...
    def text2spvec(self, query):
        ...
        """
        # Get hashed ngrams
        words = self.parse(unicodedata.normalize('NFD', query))
...
def main(dataset_file, k_tf_idf_results=10):

    ranker = TfidfDocRanker(TFIDF_PATH)

    with open(dataset_file) as f:
        dataset_list = [json.loads(line) for line in f]

    for item in dataset_list:
        item["tfidf_docs"] = ranker.closest_docs(item["claim"], k=k_tf_idf_results)
...
if __name__ == "__main__":
    ...
    dataset_file = ""
    k_tf_idf_results = 10
    main(dataset_file, k_tf_idf_results)
